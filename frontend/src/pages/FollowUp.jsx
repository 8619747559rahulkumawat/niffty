...
export default function FollowUp() {
  ...
  const fetchCampaigns = async () => {
    try { const { data } = await API.get('/campaigns'); if (data.success) setCampaigns(data.campaigns || []); } catch {}
  };

  const fetchSessions = async () => {
    try { const { data } = await API.get('/sessions'); if (data.success) setSessions(data.sessions); } catch {}
  };
  ...
  const handleDelete = async (id) => {
    ...
    try { await API.delete(`/follow-up/${id}`); fetchRules(); } catch {}
  };
