...
export default function WorkflowBuilder() {
  ...
  const toggleStatus = async (id, currentStatus) => {
    ...
    try { await API.put(`/automation/${id}/status`, { status: newStatus }); fetchFlows(); } catch {}
  };

  const deleteFlow = async (id) => {
    if (!confirm('Delete this flow?')) return;
    try { await API.delete(`/automation/${id}`); fetchFlows(); if (selectedFlow?._id === id) setSelectedFlow(null); } catch {}
  };
