...
export default function Templates() {
  ...
  const handleDelete = async (id) => {
    ...
    try { await API.delete(`/templates/${id}`); fetchTemplates(); } catch {}
  };

  const handleSubmitApproval = async (id) => {
    try { await API.post(`/templates/${id}/submit-for-approval`); fetchTemplates(); } catch {}
  };

  const handleApprove = async (id) => {
    try { await API.post(`/templates/${id}/approve`); fetchTemplates(); } catch {}
  };

  const handleReject = async (id) => {
    const reason = prompt('Rejection reason:');
    if (!reason) return;
    try { await API.post(`/templates/${id}/reject`, { reason }); fetchTemplates(); } catch {}
  };
