...
exports.sendBulk = async (req, res) => {
  try {
    ...
    for (let i = 0; i < contacts.length; i++) {
      try {
        const phone = formatPhoneNumber(contacts[i].phone);
        let result;
        if (buttons && buttons.length > 0) {
          result = await whatsappService.sendButtonMessage(sessionId, phone, message, buttons);
        } else if (mediaUrl && messageType !== 'text') {
        ...
        results.push({ phone, status: 'sent', messageId: msg._id });
        ...
        results.push({ phone: contacts[i].phone, status: 'failed', error: err.message });
      }
    }
...
exports.getMessages = async (req, res) => {
  try {
    ...
    // If user is admin, hide other users' sensitive content
    if (req.user.role === 'admin') {
